...
type
  TfrmImpressaoCozinhaCopa = class(TForm)
    ...
    qryConsultaImpressaoDATA_HORA_PEDIDO: TDateField;
...
begin
  ...
  MMImprimir.Lines.Add('</ae><e> Mesa: ' + qryConsultaImpressaoNUM_MESA.AsString);
  MMImprimir.Lines.Add('</ae><e> Cartao: ' + qryConsultaImpressaoNUM_CARTAO.AsString);
  MMImprimir.Lines.Add('</ae>Data: '+FormatDateTime('dd/mm/yy hh:nn:ss',Now));
  MMImprimir.Lines.Add('</fn>------------------------------------------------');
  MMImprimir.Lines.Add('</fn>Qte Descricao                                   ');
  MMImprimir.Lines.Add('</ae><e>'+FormatFloat('##00',qryConsultaImpressaoQTD.Value) + ' ' + qryConsultaImpressaoNOME_PRODUTO.AsString);
  MMImprimir.Lines.Add(' ');
  MMImprimir.Lines.Add(' ');
  MMImprimir.Lines.Add('</ae>Obs:' + qryConsultaImpressaoOBSERVACAO.AsString);
  MMImprimir.Lines.Add('</fn>------------------------------------------------');
  MMImprimir.Lines.Add('</ce><e>Data/Hora: '+FormatDateTime('dd/mm/yy hh:nn:ss',qryConsultaImpressaoDATA_HORA_PEDIDO.AsDateTime));
  MMImprimir.Lines.Add(' ');
  MMImprimir.Lines.Add(' ');
